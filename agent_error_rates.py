...
def discover_gate_repos(roots):
    ...
    for root in roots:
        ...
        for name in sorted(os.listdir(root)):
            ...
            if os.path.isdir(os.path.join(d, ".adversary", "reviews")):
                out.append((name, d))
    return out
...
def gate_episodes(repo_name, repo_root):
    ...
    rvw = os.path.join(repo_root, ".adversary", "reviews")
    blocks = []
    try:
        review_files = os.listdir(rvw)
    except OSError as e:
...
def archive_rows(anomalies, root_names=None):
    ...
    for sub in sorted(os.listdir(ARCHIVE)):
        ...
        # A diverging map path is an ANOMALY, not a silent fallback (r5 F3).
...
def main(argv=None):
    ...
    root_names = {os.path.normpath(root).lower(): name for name, root in repos}
    for name, root in repos:
        eps, unl, ano = gate_episodes(name, root)
